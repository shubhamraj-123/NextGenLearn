...
const CourseProgress = () => {
  ...
  // toast displaying 2 times from up
  useEffect(() => {
    if (completedSuccess && markCompleteData) {
      refetch();
      toast.success(markCompleteData.message);
    } 
    else if (inCompletedSuccess && markInCompleteData) {
      refetch();
      toast.success(markInCompleteData.message);
    }
  }, [completedSuccess, inCompletedSuccess]);
  ...
  // initialize the 1st lecture if not exist
  ...
  const handleSelectLecture = (lecture) => {
    ...
    // handleLectureProgress(lecture._id); // without seeing video it will be completed
  };
